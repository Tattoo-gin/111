...
class PS(object):
    ...
    # 对每个像素单独求解
        for i in range(self.M.shape[0]):
           I = self.M[i, :]  # 当前像素在不同光源下的强度值
           if np.linalg.norm(I) > 1e-5:  # 跳过无效或纯黑像素
            # 使用最小二乘法求解
              g, _, _, _ = np.linalg.lstsq(self.L.T, I, rcond=None)  # g 是未归一化法线
              self.N[i, :] = g  # 保存未归一化的法线

    # 对法线进行归一化
        self.N = normalize(self.N, axis=1)  # 将每个法线向量的长度归一化为 1
